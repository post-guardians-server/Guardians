...
public class RecipeCommandTabComp implements TabCompleter {
    private static List<String> argument0 = new ArrayList<>();

    public RecipeCommandTabComp() { //{"생성", "삭제", "목록", "리로드", "열기"}
        argument0.add("생성");
        argument0.add("삭제");
        argument0.add("목록");
        argument0.add("리로드");
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if(args[0].equals("삭제")) return TabCompleteUtils.searchAtSet(RecipeManager.recipesData.keySet(), args[1]);
        if(args[0].equals("생성")) {
            if(args.length==2) {
                Configure config = RecipeManager.getPathData();
                for(String key : config.getConfig().getKeys(false)) {
                    String path = config.getConfig().getString(key);
                    if(path.contains(args[1])) result.add(path);
                }
                return result;
            }
        }
